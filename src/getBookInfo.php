<?php
	...
	$exactUrl = "";
	$resultArray = array();
	function retriveBookInfo($html, $step){
		foreach($html->find('li[class=book_list_info]') as $bookInfo){
			...
			if($step == 1 && $url == $GLOBALS['exactUrl']){
				continue;
			}
			if($step == 0){
				$GLOBALS['exactUrl'] = $url;
			}
			if($DEBUG){
				echo "<br/>" . $url;
			}
			...
			array_push($GLOBALS['resultArray'], $arr);
		}
	}
	retriveBookInfo($html, 0);
	$targetUrl = $USTC_LIB_QUERY_URL . "?title=" . $bookname;
	$html = file_get_html($targetUrl);
	retriveBookInfo($html, 1);	
